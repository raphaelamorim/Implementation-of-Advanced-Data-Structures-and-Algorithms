...
public class Snc120030MultiDimesnsionalSearch {
	...
	private static Map<Long, Snc120030Product> productMap = new HashMap<Long, Snc120030Product>();
	private static Map<String, List<Snc120030Product>> productNameMap = new HashMap<String, List<Snc120030Product>>();
	private static Map<String, Snc120030Product> nameMap = new HashMap<String, Snc120030Product>();
	...
	private static float priceHike(String line) {
		...
		for (long id : keyList) {
			if (l <= id && id <= h) {
				Snc120030Product product = productMap.get(id);
...
//				sum = sum + d;
	...
	private static void sortProductLinkedList(Snc120030Product product) {
		// TODO Auto-generated method stub
		...
		for (String name : nameArray) {
		   ...
		   Snc120030Product root = nameMap.get(name);
		   if (root == null) {
	...
	private static float findPriceRange(String line) {
		...
		int count = 0;
		Snc120030Product product = nameMap.get(name);
		Snc120030Product temp = product;
	...
	private static float findMax(String line) {
		...
		String[] lineSplit = line.split(" ");
		String name = lineSplit[1];
		Snc120030Product product = nameMap.get(name.trim());
		Snc120030Product temp = product;
	...
	private static float insertItem(String line) {
		...
		if (productMap.get(id) == null) {
			Snc120030Product product = new Snc120030Product(id, nameValue, price);
...
//					List<Snc120030Product> productList = productNameMap.get(nm);
//					productList.add(product);
//				} else {
//					List<Snc120030Product> productList = new ArrayList<Snc120030Product>();
//					productList.add(product);
...
//				}
	...
	private static void insertItemInNameMap(Snc120030Product product) {
		// TODO Auto-generated method stub
		String[] nameArray = product.getName().split(" ");
		for (String nm : nameArray) {
			if (nameMap.containsKey(nm)) {
				Snc120030Product root = nameMap.get(nm);
	...
	private static void addToProductLinkedList(Snc120030Product root, Snc120030Product product, String name) {
		// TODO Auto-generated method stub
		Snc120030Product prev = null;
		Snc120030Product temp = root;
		...
		if (prev == null) {
			...
			Snc120030Product var = prev.getProductNextMap().get(name);
	...
	private static void deleteItemFromProductLinkedList(long id, String nm) {
		...
		if (root != null) {
			...
			if (prev == null) {
				...
				Snc120030Product curr = prev.getProductNextMap().get(nm);
				if (curr != null) {
					Snc120030Product next = curr.getProductNextMap().get(nm);
